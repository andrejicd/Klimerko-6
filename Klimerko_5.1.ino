 ...
 * ------------------------------------------------------------------------------------------------------------
 * Version 5.1 Stable Update: 
 * - Fixed HTTP Update memory issue (Moved logic to loop)
 * - Safer JSON parsing
 */
...
// --- NOVO: Globalna promenljiva za Update URL ---
...
// ------------------------- Device -----------------------------------------------------
String firmwareVersion = "5.1 StableUpd";
const char* firmwareVersionPortal = "<p>Firmware Version: 5.1 (Struct+Optimized+StableUpdate)</p>";
char klimerkoID[32];
...
void performFirmwareUpdate(String url) {
  ...
  // Oslobodi memoriju pre update-a
  ...
  // Povecanje timeout-a za sporije konekcije
...
void mqttCallback(char* p_topic, byte* p_payload, unsigned int p_length) {
  ...
  // Kopiraj payload u buffer sa nul-terminatorom (Safe Copy)
  ...
  if (asset == INTERVAL_ASSET) changeInterval(doc["value"]);
    ...
    String v = doc["value"].as<String>(); if (v == "true" || v == "1") shouldStartConfig = true;
     ...
     if (isNumber(v.c_str())) {
      ...
      if (url.length() > 10) {
        // SAMO POSTAVLJAMO URL, UPDATE SE DESAVA U LOOP-u DA USTEDIMO MEMORIJU
...
String wifiSignal() {
  if (!wifiConnectionLost) {
    int s = WiFi.RSSI();
    if (s < -87) return "Horrible"; else if (s <= -80) return "Bad"; else if (s <= -70) return "Decent"; else if (s <= -55) return "Good"; else return "Excellent";
  }
  return "Error";
}
...
void loop() {
  ...
  // --- IZVRSAVANJE UPDATE-A IZ PETLJE (DA USTEDIMO RAM) ---
  if (pendingUpdateUrl != "") {
     String url = pendingUpdateUrl;
     pendingUpdateUrl = ""; // Resetuj flag
